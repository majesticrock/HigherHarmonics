...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        std::cerr << "Invalid number of arguments: Use mpirun -n <threads> <path_to_executable> <configfile>"
                  << std::endl;
        return -1;
    }
...
#pragma omp parallel for firstprivate(alpha_buffer, beta_buffer) reduction(vec_plus : alphas, betas)
    for (int z = 0; z < n_z / 2; ++z) {
        ...
        for (int r = 0; r < n_kappa / 2; ++r) {
            ...
            picky_z_alpha[n_z / 2 + z] +=
                kappa_integrator::weights()[r] * integration_weight(k_z, kappa) * alpha_buffer[pick_time];
            picky_z_beta[n_z / 2 + z] +=
                kappa_integrator::weights()[r] * integration_weight(k_z, kappa) * beta_buffer[pick_time];
            ...
            picky_z_alpha[n_z / 2 - 1 - z] -=
                kappa_integrator::weights()[r] * integration_weight(k_z, kappa) * alpha_buffer[pick_time];
            picky_z_beta[n_z / 2 - 1 - z] -=
                kappa_integrator::weights()[r] * integration_weight(k_z, kappa) * beta_buffer[pick_time];
    ...
    nlohmann::json data_json{{"time", mrock::utility::time_stamp()},
    ...
    const std::string data_subdir =
        input.getString("data_dir") + "/T=" + improved_string(temperature) + "/E_F=" + improved_string(E_F) +
        "/v_F=" + improved_string(v_F) + "/band_width=" + improved_string(band_width) +
        "/field_amplitude=" + improved_string(E0) + "/photon_energy=" + improved_string(photon_energy) + "/";
    const std::string output_dir = BASE_DATA_DIR + data_subdir;

    std::filesystem::create_directories(output_dir);
    mrock::utility::save_string(data_json.dump(4), output_dir + "current_density.json.gz");
