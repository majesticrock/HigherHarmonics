...
HHG::Dispatch::PiFluxDispatcher::PiFluxDispatcher(mrock::utility::InputFileReader& input,
                                                  int N,
    ...
    : Dispatcher(N, input.getDouble("diagonal_relaxation_time")),
      ...
      system([&]() -> std::variant<Systems::PiFlux, Systems::ModifiedPiFlux> {
          if (mod_system) {
              return Systems::ModifiedPiFlux(input.getDouble("T"), input.getDouble("E_F"), input.getDouble("v_F"),
                                             input.getDouble("band_width"), Dispatcher::get_photon_energy(input),
              ...
              return Systems::PiFlux(input.getDouble("T"), input.getDouble("E_F"), input.getDouble("v_F"),
                                     input.getDouble("band_width"), Dispatcher::get_photon_energy(input),
                                     input.getDouble("diagonal_relaxation_time"),
                                     input.getDouble("offdiagonal_relaxation_time") > h_float{}
                                         ? input.getDouble("offdiagonal_relaxation_time")
    ...
    if (laser_type == continuous) {
        ...
        laser =
            std::make_unique<Laser::CosineLaser>(photon_energy, E0, laser_model_ratio, n_laser_cycles, pi * t0_offset);
        ...
        laser = std::make_unique<Laser::ExperimentalLaser>(photon_energy, E0, laser_model_ratio, t0_offset,
                                                           Laser::ExperimentalLaser::Active::A);
        time_config = {laser->t_begin, reduced_duration ? 0.75 * laser->t_end : laser->t_end, N, 50};
    } else if (laser_type == expB) {
        laser = std::make_unique<Laser::ExperimentalLaser>(photon_energy, E0, laser_model_ratio, t0_offset,
                                                           Laser::ExperimentalLaser::Active::B);
        time_config = {laser->t_begin, reduced_duration ? 0.75 * laser->t_end : laser->t_end, N, 50};
        ...
        laser = std::make_unique<Laser::PowerLawField>(photon_energy, E0, laser_model_ratio, t0_offset,
                                                       std::stod(laser_type.substr(powerlaw.size())));
        time_config = {laser->t_begin, laser->t_end, N, 50};
    } else if (laser_type == dcos) {
        laser = std::make_unique<Laser::DoubleLaser<Laser::CosineLaser>>(photon_energy, E0, laser_model_ratio,
                                                                         n_laser_cycles, t0_offset);
        time_config = {laser->t_begin, reduced_duration ? 0.75 * laser->t_end : laser->t_end, N, 50};
    } else if (laser_type == dcosA) {
        laser = std::make_unique<Laser::DoubleLaser<Laser::CosineLaser>>(
            photon_energy, E0, laser_model_ratio, n_laser_cycles, t0_offset,
            Laser::DoubleLaser<Laser::CosineLaser>::Active::A);
        time_config = {laser->t_begin, reduced_duration ? 0.75 * laser->t_end : laser->t_end, N, 50};
    } else if (laser_type == dcosB) {
        laser = std::make_unique<Laser::DoubleLaser<Laser::CosineLaser>>(
            photon_energy, E0, laser_model_ratio, n_laser_cycles, t0_offset,
            Laser::DoubleLaser<Laser::CosineLaser>::Active::B);
        time_config = {laser->t_begin, reduced_duration ? 0.75 * laser->t_end : laser->t_end, N, 50};
        ...
        laser = std::make_unique<Laser::DoubleLaser<Laser::GaussLaser>>(photon_energy, E0, laser_model_ratio,
                                                                        n_laser_cycles, t0_offset);
        time_config = {laser->t_begin, reduced_duration ? 0.75 * laser->t_end : laser->t_end, N, 50};
    } else if (laser_type == dgaussA) {
        laser = std::make_unique<Laser::DoubleLaser<Laser::GaussLaser>>(
            photon_energy, E0, laser_model_ratio, n_laser_cycles, t0_offset,
            Laser::DoubleLaser<Laser::GaussLaser>::Active::A);
        time_config = {laser->t_begin, reduced_duration ? 0.75 * laser->t_end : laser->t_end, N, 50};
    } else if (laser_type == dgaussB) {
        laser = std::make_unique<Laser::DoubleLaser<Laser::GaussLaser>>(
            photon_energy, E0, laser_model_ratio, n_laser_cycles, t0_offset,
            Laser::DoubleLaser<Laser::GaussLaser>::Active::B);
        time_config = {laser->t_begin, reduced_duration ? 0.75 * laser->t_end : laser->t_end, N, 50};
    } else {
    ...
    std::cout << "lattice constant = "
              << std::visit([&](auto& sys) { return sys.get_property_in_SI_units("d", laser->photon_energy); }, system)
              << std::endl;
    std::cout << "Hopping element = "
              << std::visit([&](auto& sys) { return sys.get_property_in_SI_units("t", laser->photon_energy); }, system)
              << std::endl;
}
...
void HHG::Dispatch::PiFluxDispatcher::compute(int rank, int n_ranks, int n_z) {
    ...
    current_density_time = std::visit(
        [&](auto& sys) { return sys.compute_current_density(laser.get(), time_config, rank, n_ranks, n_z); }, system);
...
nlohmann::json HHG::Dispatch::PiFluxDispatcher::special_information() const {
    return {{"lattice_constant",
             std::visit([&](auto& sys) { return sys.get_property_in_SI_units("d", photon_energy); }, system)},
            {"hopping_element",
             std::visit([&](auto& sys) { return sys.get_property_in_SI_units("t", photon_energy); }, system)}};
}
...
std::vector<HHG::OccupationContainer> HHG::Dispatch::PiFluxDispatcher::track_occupation_numbers(int N) const {
    ...
    std::vector<OccupationContainer> occupations =
        std::visit([&](auto& sys) { return sys.compute_occupation_numbers(laser.get(), time_config, N); }, system);
