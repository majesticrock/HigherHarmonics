...
void TrapezoidalFFT::compute(const std::vector<h_float>& input,
                             std::vector<h_float>& real_output,
                             std::vector<h_float>& imag_output) {
    ...
    for (int i = 0; i < n_fft; ++i) {
        ...
        const h_complex corrections =
            alpha_0(theta) * input[0] +
            std::conj(alpha_0(theta)) * std::polar(1.0, frequencies[i] * (t_end - t_begin)) * input[n_input - 1];
        const h_complex corrected_output =
            std::polar(delta_t * W(theta), frequencies[i] * t_begin) * h_complex(real_output[i], imag_output[i]) +
            corrections;
