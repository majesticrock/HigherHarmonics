...
namespace HHG::Laser {
DoubleCosine::DoubleCosine(h_float photon_energy,
                           h_float E_0,
                           h_float model_ratio,
                           h_float n_cycles,
                           h_float _second_laser_shift,
                           Active _active_laser /* = Active::Both*/)
    : Laser(exp_photon_energy * photon_energy, E_0, model_ratio, 0., 1.5 * 2. * HHG::pi * n_cycles, true),
...
void DoubleCosine::compute_spline() {
    ...
    std::cout << "Max shift = "
              << std::ranges::max(__temp, [](double l, double r) { return std::abs(l) < std::abs(r); }) << std::endl;
