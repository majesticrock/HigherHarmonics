...
namespace HHG::Laser {
    ...
    typedef std::array<h_float, N_experiment + N_extra + 2> A_arr;
    ...
    A_arr vector_potential(const std::array<h_float, N_experiment>& electric_field, h_float dt) 
    {
        ...
        }
        {
            ...
            const h_float third = -4 * (primeprime / (2 * __end) + (3 * prime) / (4 * __end * __end) + ret[N_experiment + 1] / (__end * __end * __end));
            const h_float fourth = - 0.25 * (3 * third / __end + 2 * primeprime / (__end * __end) + prime / (__end * __end * __end));

            auto pol = [&](h_float t) {
                return ret[N_experiment + 1] + prime * t + primeprime * t*t + third * t*t*t + fourth * t*t*t*t;
            };

            for (int i = 1; i < N_extra; ++i) {
                ret[N_experiment + 1 + i] = pol(i * dt);
            }
            ret.back() = h_float{};
        }
        return ret;
