...
#pragma omp declare reduction(vec_plus : std::vector<HHG::h_float> : std::transform(                 \
        omp_out.begin(), omp_out.end(), omp_in.begin(), omp_out.begin(), std::plus<HHG::h_float>())) \
...
#define PROGRESS_BAR_UPDATE(z_max)                                                                           \
    ...
    if (omp_get_thread_num() == 0) {                                                                         \
        mrock::utility::progress_bar(static_cast<float>(std::reduce(progresses.begin(), progresses.end())) / \
...
#define INTEGRATOR_TYPEDEF(N)                  \
    using __gauss = gauss::container<2 * (N)>; \
...
#define ERROR_INTEGRATOR_WEIGHT \
    h_float {}
#endif
...
PiFlux::PiFlux(h_float temperature,
               h_float _E_F,
               h_float _v_F,
               h_float _band_width,
               h_float _photon_energy,
               h_float _diagonal_relaxation_time,
               h_float _offdiagonal_relaxation_time)
...
void PiFlux::time_evolution_sigma(nd_vector& rhos,
                                  Laser::Laser const* const laser,
                                  const momentum_type& k,
                                  const TimeIntegrationConfig& time_config) const {
    ...
    const h_float alpha_beta_prod = 2 * std::sqrt(alpha2 * beta2);
    ...
    for (int i = 1; i <= time_config.n_measurements; ++i) {
        integrate_adaptive(make_controlled<sigma_error_stepper_type>(abs_error, rel_error), right_side, current_state,
                           t_begin, t_end, dt);
...
void PiFlux::time_evolution_diagonal_relaxation(nd_vector& rhos,
                                                Laser::Laser const* const laser,
                                                const momentum_type& k,
                                                const TimeIntegrationConfig& time_config) const {
    ...
    h_float alpha_beta_prod = 2 * std::sqrt(alpha2 * beta2);
    ...
    auto update_equilibrium_state = [&](const h_float laser_at_t) {
        ...
        const h_float xy_length = std::sqrt(sigmas[0] * sigmas[0] + sigmas[1] * sigmas[1]);
        alpha_beta_prod = 2 * std::sqrt(alpha2 * beta2) * (is_zero(xy_length) ? 1.0 : sigmas[0] / xy_length);
        alpha_beta_imag = 2 * std::sqrt(alpha2 * beta2) * (is_zero(xy_length) ? 0.0 : sigmas[1] / xy_length);
        ...
        for (auto& r : relax_to_diagonal)
            r *= (alpha_beta_diff * z_epsilon / normalization);
        ...
        for (auto& r : relax_to_offdiagonal)
            r /= normalization;
    };

    update_equilibrium_state(laser->laser_function(time_config.t_begin));

    auto right_side = [this, &k, &laser, &prefactor, &update_equilibrium_state, &relax_to_diagonal,
                       &relax_to_offdiagonal](const sigma_state_type& state, sigma_state_type& dxdt, const h_float t) {
        const sigma_state_type m = {k.cos_x, k.cos_y, std::cos(k.z - laser->laser_function(t))};
        ...
        for (std::size_t i = 0U; i < dxdt.size(); ++i) {
            dxdt[i] *= prefactor;
            dxdt[i] -=
                inverse_diagonal_relaxation_time * (state[i] / 3.0 - relax_to_diagonal[i])  // sigma^z
                + inverse_offdiagonal_relaxation_time *
                      (state[i] * (2. / 3.) - relax_to_offdiagonal[i]);  // sigma^x and sigma^y -> therefore 2*state
        }
    };
    ...
    rhos[0] = ddt_rho[2] * std::sin(k.z - laser->laser_function(t_begin)) -
              current_state[2] * std::cos(k.z - laser->laser_function(t_begin)) *
                  (laser->laser_function(t_begin + LASER_DT) - laser->laser_function(t_begin - LASER_DT)) /
                  (2 * LASER_DT);
#else
    rhos[0] = current_state[2];
#endif
    ...
    for (int i = 1; i <= time_config.n_measurements; ++i) {
        ...
        rhos[i] = ddt_rho[2] * std::sin(k.z - laser->laser_function(t_end)) -
                  current_state[2] * std::cos(k.z - laser->laser_function(t_end)) *
                      (laser->laser_function(t_end + LASER_DT) - laser->laser_function(t_end - LASER_DT)) /
                      (2 * LASER_DT);
#else
        rhos[i] = current_state[2];
#endif
...
void PiFlux::evolve_occupation_numbers(std::vector<OccupationContainer::occupation_t>& occupations,
                                       Laser::Laser const* const laser,
                                       const momentum_type& k,
    ...
    auto compute_occupations = [this, diagonal](sigma_state_type const& input, momentum_type const& k) {
        if (diagonal) {
            ...
            return OccupationContainer::occupation_t{std::sqrt(0.5 * (-diags[2] + norm(diags[0], diags[1], diags[2]))),
                                                     std::sqrt(0.5 * (diags[2] + norm(diags[0], diags[1], diags[2])))};
        }
        return OccupationContainer::occupation_t{std::sqrt(0.5 * (-input[2] + norm(input[0], input[1], input[2]))),
                                                 std::sqrt(0.5 * (input[2] + norm(input[0], input[1], input[2])))};
    };
    const h_float prefactor = 4. * hopping_element;
...
std::array<std::vector<h_float>, n_debug_points> PiFlux::compute_current_density_debug(
    Laser::Laser const* const laser,
    TimeIntegrationConfig const& time_config,
    [[maybe_unused]] const int n_z) const {
    // Debug setup
...
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < n_debug_points; ++i) {
        ...
        std::cout << "#" << i << "  k_y=" << picked[i] << ":    " << (error_evolutions[i] - time_evolutions[i]).norm()
                  << std::endl;
#else
...
std::vector<h_float> PiFlux::compute_current_density(Laser::Laser const* const laser,
                                                     TimeIntegrationConfig const& time_config,
                                                     const int rank,
                                                     const int n_ranks,
                                                     const int n_z) const {
...
h_float PiFlux::dispersion(const momentum_type& k) const {
    return std::sqrt(k.cos_x * k.cos_x + k.cos_y * k.cos_y + k.cos_z * k.cos_z);
}
...
std::array<std::array<h_float, 3>, 4> PiFlux::magnus_coefficients(const momentum_type& k,
                                                                  h_float delta_t,
                                                                  h_float t_0,
                                                                  Laser::Laser const* const laser) const {
...
void PiFlux::__time_evolution__(nd_vector& rhos,
                                Laser::Laser const* const laser,
                                const momentum_type& k,
                                const TimeIntegrationConfig& time_config) const {
...
h_float PiFlux::ic_sigma_x(const momentum_type& k,
                           h_float alpha_beta_diff,
                           h_float alpha_beta_prod,
                           h_float z_epsilon) const noexcept {
    return (alpha_beta_diff * k.cos_x * z_epsilon + alpha_beta_prod * (k.cos_y * k.cos_y + k.cos_z * z_epsilon)) /
           (dispersion(k) * z_epsilon);
}

h_float PiFlux::ic_sigma_y(const momentum_type& k,
                           h_float alpha_beta_diff,
                           h_float alpha_beta_prod,
                           h_float z_epsilon) const noexcept {
...
h_float PiFlux::ic_sigma_z(const momentum_type& k,
                           h_float alpha_beta_diff,
                           h_float alpha_beta_prod,
                           h_float z_epsilon) const noexcept {
    return (alpha_beta_diff * k.cos_z * z_epsilon - alpha_beta_prod * k.cos_x * z_epsilon) /
           (dispersion(k) * z_epsilon);
}

PiFlux::sigma_state_type PiFlux::ic_sigma(const momentum_type& k,
                                          h_float alpha_beta_diff,
                                          h_float alpha_beta_prod,
                                          h_float z_epsilon) const noexcept {
...
std::vector<h_float> PiFlux::current_density_lattice_sum(Laser::Laser const* const laser,
                                                         TimeIntegrationConfig const& time_config,
                                                         [[maybe_unused]] const int rank,
                                                         [[maybe_unused]] const int n_ranks,
                                                         const int n_z) const {
...
#pragma omp parallel for firstprivate(rhos_buffer) reduction(vec_plus : current_density_time) schedule(dynamic)
...
#endif
    {
        ...
        std::transform(current_density_time.begin(), current_density_time.end(), x_buffer.begin(),
                       current_density_time.begin(), std::plus<>());
    }
    std::cout << std::endl;
...
nd_vector PiFlux::improved_xy_integral(momentum_type& k,
...
#endif
    auto transform = [](h_float x, h_float low, h_float high) { return 0.5 * (high - low) * x + 0.5 * (high + low); };
    auto weight = [](h_float low, h_float high) { return 0.5 * (high - low); };

    auto y_integration = [&]<int __N>(h_float y_low, h_float y_high, h_float main_weight,
                                      [[maybe_unused]] h_float error_weight) {
...
std::vector<h_float> PiFlux::current_density_continuum_limit(Laser::Laser const* const laser,
                                                             TimeIntegrationConfig const& time_config,
                                                             [[maybe_unused]] const int rank,
                                                             [[maybe_unused]] const int n_ranks,
                                                             [[maybe_unused]] const int n_z) const {
...
#pragma omp parallel for private(rhos_buffer, x_buffer) reduction(vec_plus : current_density_time)
    ...
    if (rank == 0) {
        std::cout << "Jobs per rank: " << jobs_per_rank << "\nn_ranks: " << n_ranks << "\nz_range: " << z_range
                  << std::endl;
    }
    if (jobs_per_rank * n_ranks < z_range)
        ++jobs_per_rank;
...
std::vector<OccupationContainer> PiFlux::compute_occupation_numbers(Laser::Laser const* const laser,
    ...
    auto k_xy = [dx](int x) { return x * dx; };
    auto k_z = [dz](int z) { return dz * z; };
...
std::array<std::vector<h_float>, 2> PiFlux::current_per_energy(Laser::Laser const* const laser,
    ...
    auto k_xy = [dxy](int x) { return x * dxy; };
    auto k_z = [dz](int z) { return dz * z; };
...
std::array<h_float, 3> PiFlux::diagonal_sigma(sigma_state_type const& input, momentum_type const& k) const {
    ...
    return {(input[0] * (z * z - k.cos_x * k.cos_x + k.cos_y * k.cos_y) - __xy * input[1] - __xz * input[2]) / norm,
