...
ModifiedPiFlux::ModifiedPiFlux(h_float temperature,
                               h_float _E_F,
                               h_float _v_F,
                               h_float _band_width,
...
void ModifiedPiFlux::time_evolution_sigma(nd_vector& rhos,
...
void ModifiedPiFlux::time_evolution_diagonal_relaxation(nd_vector& rhos,
                                                        Laser::Laser const* const laser,
                                                        const momentum_type& k,
                                                        const TimeIntegrationConfig& time_config) const {
    ...
    auto right_side = [this, &k, &laser, &prefactor, &update_equilibrium_state, &relax_to_diagonal,
        ...
        dxdt = prefactor * m.cross(state) -
               ...
               - inverse_offdiagonal_relaxation_time *
                     (state * (2. / 3.) - relax_to_offdiagonal);  // sigma^x and sigma^y -> therefore 2*state
    };
...
std::vector<h_float> ModifiedPiFlux::compute_current_density(Laser::Laser const* const laser,
...
h_float ModifiedPiFlux::dispersion(const momentum_type& k) const {
    return std::sqrt(k.C_x * k.C_x + k.C_y * k.C_y + k.C_z * k.C_z);
}
...
void ModifiedPiFlux::__time_evolution__(nd_vector& rhos,
                                        Laser::Laser const* const laser,
                                        const momentum_type& k,
                                        const TimeIntegrationConfig& time_config) const {
...
h_float ModifiedPiFlux::ic_sigma_x(const momentum_type& k,
                                   h_float alpha_beta_diff,
                                   h_float alpha_beta_prod,
                                   h_float z_epsilon) const noexcept {
    return (alpha_beta_diff * k.C_x * z_epsilon + alpha_beta_prod * (k.C_y * k.C_y + k.C_z * z_epsilon)) /
           (dispersion(k) * z_epsilon);
}

h_float ModifiedPiFlux::ic_sigma_y(const momentum_type& k,
                                   h_float alpha_beta_diff,
                                   h_float alpha_beta_prod,
                                   h_float z_epsilon) const noexcept {
...
h_float ModifiedPiFlux::ic_sigma_z(const momentum_type& k,
                                   h_float alpha_beta_diff,
                                   h_float alpha_beta_prod,
                                   h_float z_epsilon) const noexcept {
...
ModifiedPiFlux::sigma_state_type ModifiedPiFlux::ic_sigma(const momentum_type& k,
                                                          h_float alpha_beta_diff,
                                                          h_float alpha_beta_prod,
                                                          h_float z_epsilon) const noexcept {
...
nd_vector ModifiedPiFlux::improved_xy_integral(momentum_type& k,
...
std::vector<h_float> ModifiedPiFlux::current_density_continuum_limit(Laser::Laser const* const laser,
                                                                     TimeIntegrationConfig const& time_config,
...
#pragma omp parallel for private(rhos_buffer, x_buffer) reduction(vec_plus : current_density_time)
...
#endif
    {
        ...
        for (int i = 0; i <= time_config.n_measurements; ++i) {
            x_buffer[i] *=
                z_gauss::weights[z] * std::sin(k.z - laser->laser_function(i * time_step + time_config.t_begin));
        }
...
std::vector<OccupationContainer> ModifiedPiFlux::compute_occupation_numbers(Laser::Laser const* const laser,
    ...
    auto k_xy = [dx](int x) { return x * dx - pi; };
    auto k_z = [dz](int z) { return dz * z - pi; };
...
#pragma omp parallel for
    for (int i = 0; i < N * N; ++i) {
        ...
        auto right_side = [this, &k, &laser, &prefactor, &update_equilibrium_state, &relax_to_diagonal,
                           &relax_to_offdiagonal](const sigma_state_type& state, sigma_state_type& dxdt,
                                                  const h_float t) {
            const sigma_state_type m = {k.C_x, k.C_y, std::cos(k.z - laser->laser_function(t))};
            update_equilibrium_state(laser->laser_function(t));
        ...
        computed_occupations[0](x, coordinate_shift(z, laser->laser_function(t_begin))) =
            occupations(current_state, shifted_k);

        runge_kutta4<sigma_state_type> stepper;

        for (int i = 1; i <= time_config.n_measurements; ++i) {
            integrate_adaptive(make_controlled<sigma_error_stepper_type>(abs_error, rel_error), right_side,
                               current_state, t_begin, t_end, dt);
            // integrate_const(stepper, right_side, current_state, t_begin, t_end, dt);
            computed_occupations[i](x, coordinate_shift(z, laser->laser_function(t_begin))) =
                occupations(current_state, shifted_k);
...
std::pair<std::vector<h_float>, std::vector<h_float>> ModifiedPiFlux::current_per_energy(
    Laser::Laser const* const laser,
    TimeIntegrationConfig const& time_config,
    const int N) const {
    ...
    auto k_xy = [dxy](int x) { return x * dxy - pi; };
    auto k_z = [dz](int z) { return dz * z - pi; };
...
std::array<h_float, 3> ModifiedPiFlux::diagonal_sigma(sigma_state_type const& input, momentum_type const& k) const {
    ...
    return {(input(0) * (z * z - k.C_x * k.C_x + k.C_y * k.C_y) - __xy * input(1) - __xz * input(2)) / norm,
