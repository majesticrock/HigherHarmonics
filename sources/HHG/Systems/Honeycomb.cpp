...
Honeycomb::momentum_type::momentum_type(h_float x, h_float y) noexcept : x(x), y(y) {
    this->set_gamma();
}
...
Honeycomb::Honeycomb(h_float temperature,
                     h_float _E_F,
                     h_float _v_F,
                     h_float _band_width,
...
std::array<std::vector<h_float>, n_debug_points> Honeycomb::compute_current_density_debug(
    [[maybe_unused]] Laser::Laser const* const laser,
...
void Honeycomb::time_evolution_sigma(nd_vector& rho_x,
                                     nd_vector& rho_y,
    ...
    auto right_side = [this, &k, &laser, &prefactor](const sigma_state_type& state, sigma_state_type& dxdt,
                                                     const h_float t) {
        const h_complex buffer =
            k.shifted_gamma(cos_theta * laser->laser_function(t), sin_theta * laser->laser_function(t));
...
void Honeycomb::time_evolution_diagonal_relaxation(nd_vector& rho_x,
                                                   nd_vector& rho_y,
                                                   Laser::Laser const* const laser,
                                                   const momentum_type& k,
                                                   const TimeIntegrationConfig& time_config) const {
    ...
    auto update_equilibrium_state = [this, &k, &equilibrium_state](const h_float laser_at_t) {
        // Something is not quite right - a constant (finite) vector potential currently causes a constant current
        // density.
    ...
    auto right_side = [this, &k, &laser, &prefactor, &equilibrium_state, &update_equilibrium_state](
...
void Honeycomb::__time_evolution__(nd_vector& rho_x,
                                   nd_vector& rho_y,
                                   Laser::Laser const* const laser,
                                   const momentum_type& k,
                                   const TimeIntegrationConfig& time_config) const {
...
std::vector<h_float> Honeycomb::compute_current_density(Laser::Laser const* const laser,
                                                        ...
                                                        [[maybe_unused]] const int n_ranks) const {
    constexpr h_float min_x = -2 * pi / 3;
    constexpr h_float max_x = 2 * pi / 3;
    auto bound_y = [](h_float x) { return (4 * pi / (3 * sqrt_3)) - (std::abs(x) / sqrt_3); };
...
#pragma omp parallel for private(rho_x, rho_y) reduction(vec_plus : buffer_x) reduction(vec_plus : buffer_y)
...
#endif
    {
        ...
        for (int y = 0; y < N_k; ++y) {
            const h_float transform_weight =
                weight(min_x, max_x) * weight(-curr_max_y, curr_max_y) * __gauss::weights[x] * __gauss::weights[y];
            ...
            for (int i = 0; i <= time_config.n_measurements; ++i) {
                const auto grad_gamma =
                    k.shifted_grad_gamma(cos_theta * laser->laser_function(time_config.t_begin + i * time_step),
                                         sin_theta * laser->laser_function(time_config.t_begin + i * time_step));
    ...
    // I assume that the current density will vanish orthogonal to the laser polarization, but this is yet to be
    // verified 28.05.2025: Seems to be correct
    for (int i = 0; i <= time_config.n_measurements; ++i) {
...
Eigen::Vector<HHG::h_float, 3> Honeycomb::ic_sigma(const momentum_type& k,
                                                   h_float alpha_beta_diff,
                                                   h_float alpha_beta_prod) const noexcept {
    assert(!k.is_dirac_point());
    return Eigen::Vector<HHG::h_float, 3>{-alpha_beta_diff * k.gamma.real() / std::abs(k.gamma),
                                          alpha_beta_diff * k.gamma.imag() / std::abs(k.gamma), -alpha_beta_prod};
}
}  // namespace HHG::Systems