...
DiracSystem::DiracSystem(h_float temperature,
                         h_float _E_F,
                         h_float _v_F,
                         h_float _band_width,
...
void DiracSystem::time_evolution(nd_vector& rhos,
    ...
    state_type current_state = {fermi_function(E_F + dispersion(k_z, kappa), beta),
                                fermi_function(E_F - dispersion(k_z, kappa), beta)};
    ...
    for (int i = 1; i <= time_config.n_measurements; ++i) {
        integrate_adaptive(make_controlled<error_stepper_type>(abs_error, rel_error), right_side, current_state,
                           t_begin, t_end, dt);
...
void DiracSystem::time_evolution_complex(std::vector<h_complex>& alphas,
                                         std::vector<h_complex>& betas,
                                         Laser::Laser const* const laser,
                                         h_float k_z,
                                         h_float kappa,
                                         const TimeIntegrationConfig& time_config) const {
    ...
    state_type current_state = {static_cast<h_float>(E_F > -dispersion(k_z, kappa)),
                                static_cast<h_float>(E_F > dispersion(k_z, kappa))};
    const h_float inital_norm = current_state.norm();
    const h_float measure_every = time_config.measure_every();
    const h_float dt = time_config.dt();
...
void DiracSystem::time_evolution_sigma(nd_vector& rhos,
                                       Laser::Laser const* const laser,
                                       h_float k_z,
                                       h_float kappa,
                                       const TimeIntegrationConfig& time_config) const {
    const h_float magnitude_k = norm(k_z, kappa);
    auto right_side = [&laser, &k_z, &kappa, &magnitude_k](const sigma_state_type& state, sigma_state_type& dxdt,
                                                           const h_float t) {
...
void DiracSystem::time_evolution_magnus(nd_vector& rhos,
                                        Laser::Laser const* const laser,
                                        h_float k_z,
                                        h_float kappa,
                                        const TimeIntegrationConfig& time_config) const {
...
void DiracSystem::time_evolution_diagonal_relaxation(nd_vector& rhos,
                                                     Laser::Laser const* const laser,
                                                     h_float k_z,
                                                     h_float kappa,
                                                     const TimeIntegrationConfig& time_config) const {
    ...
    auto right_side = [this, &laser, &k_z, &kappa, &magnitude_k, &initial_state](
                          const sigma_state_type& state, sigma_state_type& dxdt, const h_float t) {
        const h_float vector_potential = laser->laser_function(t);
        const h_float m_x = vector_potential * 2.0 * kappa / magnitude_k;
        const h_float m_z = 2.0 * (magnitude_k - vector_potential * k_z / magnitude_k);
...
h_float DiracSystem::kappa_integration_upper_limit(h_float k_z) const {
    ...
    return std::sqrt(max_kappa_compare - k_z_squared);
}
...
std::vector<h_float> DiracSystem::compute_current_density(Laser::Laser const* const laser,
                                                          TimeIntegrationConfig const& time_config,
                                                          [[maybe_unused]] const int rank,
                                                          [[maybe_unused]] const int n_ranks,
                                                          const int n_z,
                                                          const int n_kappa,
                                                          const h_float kappa_threshold) const {
    ...
    auto integration_weight = [](h_float k_z, h_float kappa) { return k_z * kappa / norm(k_z, kappa); };
...
#pragma omp parallel for firstprivate(rhos_buffer) reduction(vec_plus : current_density_time) schedule(dynamic)
    ...
    if (jobs_per_rank * n_ranks < n_z - 1)
        ++jobs_per_rank;
...
#endif
    {
        ...
        if (z == n_z / 2)
            continue;  // integration weight (z = 0) = 0
        ...
        auto kappa_result = integrator.split_integrate<100>(
            kappa_integrand, h_float{}, kappa_integration_upper_limit(k_z), n_kappa, kappa_threshold,
            mrock::utility::Numerics::vector_elementwise_error<nd_vector, h_float, false>(),
            nd_vector::Zero(time_config.n_measurements + 1));

        std::transform(current_density_time.begin(), current_density_time.end(), kappa_result.begin(),
...
std::array<std::vector<h_float>, n_debug_points> DiracSystem::compute_current_density_debug(
    Laser::Laser const* const laser,
    TimeIntegrationConfig const& time_config,
    const int n_z,
    [[maybe_unused]] const int n_kappa,
    [[maybe_unused]] const h_float kappa_threshold) const {
    auto integration_weight = [](h_float k_z, h_float kappa) { return k_z * kappa / norm(k_z, kappa); };
    const auto delta_z = 2 * z_integration_upper_limit() / n_z;
...
std::vector<h_float> DiracSystem::compute_current_density_diagonal_relaxation(Laser::Laser const* const laser,
                                                                              TimeIntegrationConfig const& time_config,
                                                                              [[maybe_unused]] const int rank,
                                                                              [[maybe_unused]] const int n_ranks,
                                                                              const int n_z,
...
std::array<std::vector<h_float>, n_debug_points> DiracSystem::compute_current_density_diagonal_relaxation_debug(
    Laser::Laser const* const laser,
    TimeIntegrationConfig const& time_config,
    const int n_z,
    [[maybe_unused]] const int n_kappa /*  = 20 */,
    [[maybe_unused]] const h_float kappa_threshold /*  = 1e-3 */) const {
    auto integration_weight = [](h_float k_z, h_float kappa) { return k_z * kappa / norm(k_z, kappa); };
    const auto delta_z = 2 * z_integration_upper_limit() / n_z;
    ...
    for (int i = 0; i < n_debug_points; ++i) {
        picked[i] = (i + 1) * picked_kappa_max / (n_debug_points + 1);
        time_evolution_diagonal_relaxation(time_evolutions[i], laser, (picked_z - n_z / 2) * delta_z, picked[i],
                                           time_config);
        time_evolutions[i] *= integration_weight((picked_z - n_z / 2) * delta_z, picked[i]);
    }
    // end debug setup
...
h_float DiracSystem::max_kappa(h_float k_z) const {
    return (k_z > max_kappa_compare ? std::sqrt(max_kappa_compare - k_z) : h_float{});
}

DiracSystem::r_matrix DiracSystem::basic_transformation(h_float k_z, h_float kappa) const {
    const h_float magnitude_k = norm(k_z, kappa);
    if (is_zero(magnitude_k))
        return r_matrix::Zero();
    const h_float N = 1. / std::sqrt(2 * magnitude_k * (k_z + magnitude_k));

    const r_matrix V{{N * (k_z + magnitude_k), -N * kappa}, {N * kappa, N * (k_z + magnitude_k)}};
    return V;
}
...
DiracSystem::c_matrix DiracSystem::dynamical_matrix(h_float k_z, h_float kappa, h_float vector_potential) const {
    ...
    const c_matrix A{{-factor * k_z + magnitude_k, -factor * kappa}, {-factor * kappa, factor * k_z - magnitude_k}};
...
DiracSystem::r_matrix DiracSystem::real_dynamical_matrix(h_float k_z, h_float kappa, h_float vector_potential) const {
    const h_float magnitude_k = norm(k_z, kappa);
    const h_float factor = vector_potential / magnitude_k;
    return r_matrix{{-factor * k_z + magnitude_k, -factor * kappa}, {-factor * kappa, factor * k_z - magnitude_k}};
}
}  // namespace HHG::Systems