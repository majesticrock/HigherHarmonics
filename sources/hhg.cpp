...
#define OUTPUT_DATA_DIR "../../data/hhg/"
#endif
...
int main(int argc, char** argv) {
     ...
     *  PiFlux: 2^10 is the minimum for v_F = 1.5e3 m/s - but the band width is also very imporant; large band width is
     * apparently good
     */
    ...
    const std::string data_subdir =
        input.getString("data_dir") + "/" + laser_type + "_laser" + +"/" + system_type +
        +"/T=" + improved_string(temperature) + "/E_F=" + improved_string(E_F) + "/v_F=" + improved_string(v_F) +
        "/band_width=" + improved_string(band_width) + "/field_amplitude=" + improved_string(E0) +
    ...
    const std::string output_dir = std::string(OUTPUT_DATA_DIR) + data_subdir;
    ...
    if (occupations != "no") {
        ...
        for (int t = 0; t < N_time + int(laser_type != "continuous"); ++t) {
        ...
        for (std::size_t i = 0U; i < laser_function.size(); i++) {
            laser_function[i] =
                piflux_d.laser->laser_function(piflux_d.time_config.t_begin + i * piflux_d.time_config.measure_every());
        }
        ...
        mrock::utility::save_string(data_json.dump(4), output_dir + "occupations.json.gz");
        ...
        mrock::utility::save_string(info_json.dump(4), output_dir + "occupations_metadata.json.gz");
    }
    if (occupations == "only")
        return EXIT;
    ...
    if (split_current != "no") {
        ...
        mrock::utility::save_string(data_json.dump(4), output_dir + "split_current.json.gz");

        // Generate metadata
        nlohmann::json info_json = mrock::utility::generate_json<HigherHarmonics::info>("hhg_");
        info_json.update(mrock::utility::generate_json<mrock::info>("mrock_"));
        mrock::utility::save_string(info_json.dump(4), output_dir + "split_current_metadata.json.gz");
    }
    if (split_current == "only")
        return EXIT;
    ...
    if (debug_data != "only")
        dispatcher->compute(rank, n_ranks, n_z);
    if (debug_data != "no")
        dispatcher->debug(n_z);

#ifndef NO_MPI
    std::vector<h_float> current_density_time(dispatcher->current_density_time.size());
    MPI_Reduce(dispatcher->current_density_time.data(), current_density_time.data(),
               dispatcher->current_density_time.size(), MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
#else
    ...
    for (std::size_t i = 0U; i < laser_function.size(); i++) {
        laser_function[i] = dispatcher->laser->laser_function(dispatcher->time_config.t_begin +
                                                              i * dispatcher->time_config.measure_every());
    }
    // We do not need to output the zero padding
    std::vector<h_float> current_density_time_output(current_density_time.begin(),
                                                     current_density_time.begin() + N + 1);
    nlohmann::json data_json{
        {"time", mrock::utility::time_stamp()},
        {"laser_function", laser_function},
        {"N", N},
    ...
    nlohmann::json info_json = mrock::utility::generate_json<HigherHarmonics::info>("hhg_");
    info_json.update(mrock::utility::generate_json<mrock::info>("mrock_"));
    mrock::utility::save_string(info_json.dump(4), output_dir + "metadata.json.gz");

    if (debug_data == "no")
        return EXIT;

    // Debug output
    nlohmann::json debug_json = {{"time", mrock::utility::time_stamp()},
                                 {"frequencies", frequencies},
                                 {"time_evolutions", dispatcher->time_evolutions},
                                 {"debug_fft_real", debug_fft_real},
                                 {"debug_fft_imag", debug_fft_imag}};
    mrock::utility::save_string(debug_json.dump(4), output_dir + "time_evolution.json.gz");
